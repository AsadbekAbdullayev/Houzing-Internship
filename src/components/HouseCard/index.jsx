...
export const HouseCard = ({ info, mr, ml, margin }) => {
  ...
  const Selected = (id)=>{
    navigate(`/detail?house_id=${id}`);
    };
  return (
    <Container mr={mr} ml={ml} margin={margin} onClick={()=>Selected(info.id)}>
      ...
      <InfoWrapper>
        ...
        <Info.Title>{info?.description || 'description'} Category:{info?.category?.name}</Info.Title>
        <Info.Text>
